...
class MainController(CRUDMixin):
    def __init__(self, session, authenticated_user, cli_view):
        ...
        self.client_controller = ClientController(session, self.main_view)
        self.contract_controller = ContractController(session, self.main_view, cli_view)
        self.user_controller = UserController(session, self.main_view)
        self.event_controller = EventController(session, self.main_view)
    ...
    def handle_logout(self):
        if self.cli_view.print_logout_view() == "y":
            self.session.close()
